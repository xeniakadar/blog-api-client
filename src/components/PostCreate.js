      ...
      if (response.ok) {
        ...
        for (let error of errorData.errors) {
          if (error.msg === "Title must be specified") {
            ...
            error.msg === "Your blogpost must be at least 5 characters long"
          ) {
  ...
  return (
    <div className="blogpost-detail-container max-h-full md:w-9/10  lg:max-w-2xl mx-auto p-3 min-h-screen">
      ...
      <form className="flex flex-col mx-4 ">
        {titleError && (
          <h1 className="text-red-800 dark:text-red-300 font-bold">
            {titleError}
          </h1>
        )}
        ...
        <button
          className="btn-submit mt-4 w-full md:w-36 border-2 bg-sky-800 text-white rounded-xl px-3 py-2 hover:bg-white hover:text-sky-900 hover:border-sky-900 ease-in-out duration-300"
          onClick={saveDraft}
        >
          Save Draft
        </button>
