...
export default function PostHomepage({title, text, username, timestamp, topic, blogpostId}) {
  ...
  return (
    <div className="blogpost-container w-full md:w-9/10 lg:w-9/10 xl:w-88 xl:max-w-6xl mx-auto mb-3 rounded-2xl p-3" style={topic ? { backgroundColor: colorMap[topic].color } : {}}>
      <div className='post'>
        {topic &&
        <h2 className='title bg-sky-950 ease-in-out duration-300 hover:scale-105  w-max py-1 px-3 rounded-2xl md:text-lg font-secondary font-bold ' style={topic ? { backgroundColor: colorDarkMap[topic].color, color: colorMap[topic].color } : {}}>{topic.toUpperCase()} <span>/</span> <span className='text-white'> <Link to={`${blogpostId}`}>{title.toUpperCase()}</Link></span> </h2>
        }
        {!topic &&
