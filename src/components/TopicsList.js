...
export default function Topics() {
  ...
  return (
    <div className='topics-container p-2 m-2 mb-0 z-10'>
        <h1 className='font-sans dark:text-white pb-2 z-0 text-3xl md:text-5xl font-extrabold w-full md:w-9/10 lg:w-9/10 xl:w-88 xl:max-w-6xl mx-auto'>Destinations</h1>
        {topics.map((topic) => {
            ...
            return (
              <Link key={topic._id} to={`${topic._id}`} className="">
                <div className="zoom-effect-container relative overflow-hidden rounded-3xl my-3 w-full md:w-9/10 lg:w-9/10 xl:w-88 xl:max-w-6xl mx-auto" style={{ height: '450px' }}>
                  ...
                  <h2 className="topic--bg font-secondary text-3xl font-bold absolute dark:text-white bottom-0 left-0 z-10 p-2">{topic.title}</h2>
                </div>
              </Link>
            )
        })}
